export const avance = {
  theme: "dark",
  ticker: [
    "BOOTCAMP AVANCÉ",
    "ADMISSION SUR PORTFOLIO",
    "12 SEMAINES · PARIS",
    "SESSION JANVIER 2027",
  ],
  breadcrumbLabel: "BOOTCAMP AVANCÉ",
  kicker: "NIVEAU 02 — AVANCÉ",
  kickerRight: "ADMISSION SUR PORTFOLIO",
  titleLine1: "Viser",
  titlePrefix: "l'",
  titleHighlight: "excellence.",
  introStrong: "Devenir un Artiste 3D spécialisé dans la publicité de luxe.",
  introRest:
    " En 12 semaines, on pousse votre travail au niveau attendu par les maisons : production publicitaire, shading premium, lighting avancé et briefs clients réels qui décrochent un studio.",
  ctaPrimary: { label: "→ Soumettre mon portfolio", href: "/candidature" },
  ctaSecondary: {
    label: "Débutant ? Voir le bootcamp novice →",
    href: "/programmes/bootcamp-novice",
  },
  heroCaption: {
    left: "FIG. 01 — TRAVAIL PROMO AVANCÉ, BRIEF CLIENT",
    right: "1920×1080",
  },
  facts: [
    { label: "DURÉE", value: "12 sem." },
    { label: "RYTHME", value: "Plein temps" },
    { label: "PRÉREQUIS", value: "Portfolio" },
    { label: "RENTRÉE", value: "Janv. 2027" },
    { label: "TARIF", value: "7 900 €", highlight: true },
  ],
  pourQui: {
    title: "Sélection sur portfolio.",
    intro:
      "Le bootcamp Avancé ne reprend pas les fondamentaux. On attend une maîtrise réelle d'un logiciel 3D et un portfolio existant que l'on va faire passer un cap.",
    list: [
      "Maîtrise d'au moins un logiciel 3D (Blender, Maya, Modo…)",
      "Un portfolio ou des projets personnels à présenter",
      "Diplômés du bootcamp Novice ou équivalent",
      "Objectif clair : intégrer un studio CGI luxe",
    ],
  },
  modulesTitle: "Le niveau agence.",
  modules: [
    {
      num: "01",
      title: "Production Publicitaire",
      tags: [
        "Hero Film",
        "CGI Advertising",
        "Luxury Visual Storytelling",
        "Campagne publicitaire complète",
        "Workflow studio",
      ],
    },
    {
      num: "02",
      title: "Modélisation Avancée",
      tags: [
        "Hard Surface Expert",
        "High Poly Modeling",
        "Optimisation Production",
        "Packaging complexe",
        "Product Design",
      ],
    },
    {
      num: "03",
      title: "Shading Premium",
      tags: [
        "Procedural Shading",
        "Micro Details",
        "Fingerprints",
        "Condensation",
        "Liquid Shader",
        "Cosmetic Materials",
      ],
    },
    {
      num: "04",
      title: "Éclairage avancé",
      tags: [
        "Advanced Lighting",
        "Luxury Lighting",
        "Rim Light",
        "Dramatic Lighting",
        "Product Highlight",
        "Mood Lighting",
      ],
    },
    {
      num: "05",
      title: "Simulations Houdini / Blender",
      tags: [
        "FLIP Fluids",
        "White Water",
        "Particles",
        "Smoke Simulation",
        "Pyro",
        "Splash Cosmetics",
        "Morphing",
        "Rigid Body Dynamics",
      ],
    },
    {
      num: "06",
      title: "Animation Avancée",
      tags: [
        "Hero Animation",
        "Camera Rig",
        "Slow Motion",
        "Motion Graphics",
        "High-End Product Reveal",
        "Commercial Timing",
      ],
    },
    {
      num: "07",
      title: "Compositing Professionnel",
      tags: [
        "Multi Pass Compositing",
        "Cryptomatte",
        "Z-Depth",
        "Lens Effects",
        "Color Pipeline",
        "ACES Workflow",
      ],
    },
    {
      num: "08",
      title: "Direction Artistique",
      tags: [
        "Luxury Brand Codes",
        "Storytelling Produit",
        "Analyse des campagnes YSL, Chanel, Dior…",
        "Moodboard",
        "Construction d'une DA premium",
      ],
    },
    {
      num: "09",
      title: "Pipeline Agence",
      tags: [
        "Gestion d'un brief client",
        "Organisation d'une production",
        "Optimisation des scènes",
        "Livraison client",
        "Feedback Workflow",
      ],
    },
    {
      num: "10",
      title: "Portfolio & Freelance",
      tags: [
        "Portfolio professionnel",
        "Positionnement Freelance",
        "Tarification",
        "Relation client",
        "Réponse à appel d'offres",
        "Personal Branding",
      ],
    },
  ],
  weeks: [
    {
      weeks: "SEM. 01 — 02",
      title: "Lookdev & shading avancé",
      desc: "Audit de votre portfolio, puis montée en gamme des shaders : réalisme, imperfections, micro-détails.",
    },
    {
      weeks: "SEM. 03 — 04",
      title: "Texturing & finitions luxe",
      desc: "Substance en profondeur, sérigraphie, gaufrage, dorure à chaud. Les finitions qui font le haut de gamme.",
    },
    {
      weeks: "SEM. 05 — 06",
      title: "Direction artistique",
      desc: "Lecture des codes des maisons, composition, mise en scène du produit comme un bijou. Premier brief client.",
    },
    {
      weeks: "SEM. 07 — 08",
      title: "Éclairage & animation",
      desc: "Lumière de campagne, turntables et reveals animés. On passe de l'image fixe au mouvement.",
    },
    {
      weeks: "SEM. 09 — 10",
      title: "Simulations & effets",
      desc: "Fluides, particules et morphing sous Houdini et Blender : condensation, éclaboussures et textures cosmétiques en mouvement.",
    },
    {
      weeks: "SEM. 11 — 12",
      title: "Brief final & reel",
      desc: "Deuxième brief client réel, montage du reel, finalisation du book et mise en relation studios.",
    },
  ],
  toolsIntro:
    "On travaille sur le pipeline réel des studios CGI beauté, du shading avancé au rendu production.",
  tools: [
    { name: "Substance", use: "TEXTURING" },
    { name: "Redshift", use: "RENDU PRODUCTION" },
    { name: "Houdini", use: "SIMULATION" },
    { name: "Nuke", use: "COMPOSITING" },
  ],
  resultatTitle: "Prêt pour l'agence.",
  resultatMediaFirst: false,
  resultatCaption: { left: "FIG. 02 — REEL DE SORTIE", right: "NIVEAU STUDIO" },
  checklist: [
    "Produire une publicité 3D de niveau agence",
    "Réaliser des simulations haut de gamme",
    "Construire un portfolio orienté luxe",
    "Répondre à un brief client réel",
    "Être prêt à intégrer une agence, un studio ou travailler en freelance",
  ],
  tarif: {
    label: "TARIF DU BOOTCAMP AVANCÉ",
    price: "7 900 €",
    desc: "Tout compris : pipeline studio, 2 briefs clients réels, mentorat individuel et mise en relation.",
    financement: ["Acompte de 50 % à l'inscription"],
  },
  cta: {
    title: "Montrez-nous votre travail.",
    label: "→ Soumettre mon portfolio",
  },
  footerExtraLink: {
    label: "Bootcamp Novice",
    href: "/programmes/bootcamp-novice",
  },
  footerTopAccent: false,
};
